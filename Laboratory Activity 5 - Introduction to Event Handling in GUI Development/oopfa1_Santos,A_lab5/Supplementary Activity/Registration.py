...
class MyWindow:
    ...
    def submit(self):
        put_data = {"First Name": self.Entry1.get(),
                    "Last Name": self.Entry2.get(),
                    "Username": self.Entry3.get(),
                    "Password": self.Entry4.get(),
                    "Email": self.Entry5.get(),
                    "Contact No.": self.Entry6.get()}

        for entry, val in put_data.items():
            if not val:
                messagebox.showwarning("Error!!", f"{entry} is required, fill that blank!")
                return
        messagebox.showinfo("The file is a success", "Now, it is successfully saved!")

        with open("~Registration Form~.txt", "a") as fl:
            fl.write(" ".join(["First Name:", self.Entry1.get(),
                                 "\nLast Name:", self.Entry2.get(),
                                 "\nUsername:", self.Entry3.get(),
                                 "\nPassword:", self.Entry4.get(),
                                 "\nEmail:", self.Entry5.get(),
